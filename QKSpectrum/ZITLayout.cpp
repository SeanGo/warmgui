...
void CZITLayout::SetConfig(CWarmguiConfig* config, const char* conf_str)
{
    ILayout::SetConfig(config, conf_str);
}

void CZITLayout::Disposal(WARMGUI::IAtelier* atelier, RECT& rectAtelier)
{
	RECT rect = rectAtelier, rectRealAtelier = rectAtelier;
	if (RectWidth(rect) <= 0 || RectHeight(rect) <= 0)
		return;

	CZITAtelier* zit = (CZITAtelier*)atelier;
