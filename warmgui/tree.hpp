...
template<class T>
class WARMGUI_API tree_node_ { // size: 5*4=20 bytes (on 32 bit arch), can be reduced by 8.
	public:
		tree_node_();
		tree_node_(const T&);
...
template <class T, class WARMGUI_API tree_node_allocator = WARMGUI_API std::allocator<tree_node_<T> > >
class WARMGUI_API tree {
	protected:
	...
	public:
      ...
      leaf_iterator   end_leaf(const iterator_base& top) const;
		...
		iterator_base find(const T& t) {
			sibling_iterator iter = begin();
			for( ; iter != end(); iter++) {
				if (*iter == t)
					break;
			}
