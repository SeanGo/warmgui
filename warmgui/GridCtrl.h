...
#if !defined (__grid_ctrl_h__)
#define __grid_ctrl_h__
...
typedef std::vector<CGridCellBase*> GRID_ROW;
...
public:
    ...
    BOOL Create(const RECT& rect, HWND parent, UINT nID,
...
public:
    ...
    inline SIZE_u GetCellTextExtent(int nRow, int nCol)  {
        SIZE_u size;
        return size;
    ////SEAN SEAN SEAN     return GetTextExtent(nRow, nCol, GetItemText(nRow,nCol));
    }
    ...
    int GetSelectedCount() const                  { return (int)0; } ////SEAN SEAN SEAN m_SelectedCellMap.GetCount(); }
...
public:
    ...
    LPCTSTR GetItemText(int nRow, int nCol) const;
    ...
    ////SEAN SEAN SEAN BOOL SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass);
    ////SEAN SEAN SEAN BOOL SetDefaultCellType( CRuntimeClass* pRuntimeClass);
...
///////////////////////////////////////////////////////////////////////////////////
 ...
 public:
    ...
    virtual DROPEFFECT OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, POINT point);
    virtual DROPEFFECT OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, POINT point);
    virtual void OnDragLeave();
    virtual BOOL OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, POINT point);
#endif
...
protected:
    ...
    ////SEAN SEAN SEAN void EraseBkgnd(CDC* pDC);
    ...
    POINT GetPointClicked(int nRow, int nCol, const POINT& point);
...
protected:
...
#ifndef GRIDCONTROL_NO_DRAGDROP
    // Drag n' drop
    virtual CImageList* CreateDragImage(POINT *pHotSpot);    // no longer necessary
#endif
    ...
    // Editing
    virtual void  OnEditCell(int nRow, int nCol, POINT& point, UINT nChar);
    virtual void  OnEndEditCell(int nRow, int nCol, LPCTSTR str);
...
////SEAN SEAN SEAN     CUIntArray  m_arRowHeights, m_arColWidths;
    int         m_nVScrollMax, m_nHScrollMax;

    // Fonts and images
////SEAN SEAN SEAN     CRuntimeClass*   m_pRtcDefault; // determines kind of Grid Cell created by default
    ...
    LOGFONT          m_PrinterFont;  // for the printer
    ...
    ////SEAN SEAN SEAN CMap<DWORD,DWORD, CCellID, CCellID&> m_SelectedCellMap, m_PrevSelectedCellMap;
    ...
    SIZE_u      m_CharSize;
    int         m_nPageHeight;
    SIZE_u      m_LogicalPageSize,      // Page size in gridctrl units.
...
protected:
    ...
    //{{AFX_MSG(CGridCtrl)
    msg_fun void OnPaint();
    ////SEAN SEAN SEAN msg_fun void OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar);
    ////SEAN SEAN SEAN msg_fun void OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar);
    msg_fun void OnSize(UINT nType, int cx, int cy);
    msg_fun void OnLButtonUp(UINT nFlags,  POINT point);
    msg_fun void OnLButtonDown(UINT nFlags, POINT point);
    msg_fun void OnMouseMove(UINT nFlags, POINT point);
    msg_fun void OnTimer(UINT_PTR nIDEvent);
    msg_fun UINT OnGetDlgCode();
    msg_fun void OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags);
	msg_fun void OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags);
    msg_fun void OnChar(UINT nChar, UINT nRepCnt, UINT nFlags);
    msg_fun void OnLButtonDblClk(UINT nFlags, POINT point);
    ////SEAN SEAN SEAN msg_fun BOOL OnEraseBkgnd(CDC* pDC);
    msg_fun void OnSysKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags);
    ////SEAN SEAN SEAN msg_fun void OnUpdateEditSelectAll(CCmdUI* pCmdUI);
    //}}AFX_MSG
#ifndef _WIN32_WCE_NO_CURSOR
    msg_fun BOOL OnSetCursor(HWND pWnd, UINT nHitTest, UINT message);
#endif
#ifndef _WIN32_WCE
    msg_fun void OnRButtonDown(UINT nFlags, POINT point);
    msg_fun void OnRButtonUp(UINT nFlags, POINT point);    // EFW - Added
    msg_fun void OnSysColorChange();
#endif
#ifndef _WIN32_WCE_NO_CURSOR
    msg_fun void OnCaptureChanged(HWND pWnd);
#endif
#ifndef GRIDCONTROL_NO_CLIPBOARD
    msg_fun void OnUpdateEditCopy(CCmdUI* pCmdUI);
    msg_fun void OnUpdateEditCut(CCmdUI* pCmdUI);
    msg_fun void OnUpdateEditPaste(CCmdUI* pCmdUI);
#endif

    msg_fun void OnSettingChange(UINT uFlags, LPCTSTR lpszSection);
    msg_fun BOOL OnMouseWheel(UINT nFlags, short zDelta, POINT pt);
    msg_fun LRESULT OnSetFont(WPARAM hFont, LPARAM lParam);
    msg_fun LRESULT OnGetFont(WPARAM hFont, LPARAM lParam);
    msg_fun LRESULT OnImeChar(WPARAM wCharCode, LPARAM lParam);
    msg_fun void OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult);
...
inline BOOL CGridCtrl::SetCell(int nRow, int nCol, CGridCellBase* pCell)
{
    ...
    ////SEAN SEAN SEAN pRow[nCol] = pCell;
